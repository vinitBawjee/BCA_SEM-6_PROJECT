...
const Update_Profile = () => {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            ...
            if (!userId) return;
            try {
                const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/seller/user/${userId}`);
        ...
        if (validateUpdate()) {
            ...
            try {
                const res = await axios.post(
                    `${import.meta.env.VITE_API_URL}/seller/update-user`,
                    formDataObj,
                    { 
                        headers: { 
    ...
    const handlePasswordSubmit = async () => {
        ...
        try {
            const res = await axios.post(
                `${import.meta.env.VITE_API_URL}/seller/change-password`, 
                changePassword, 
    ...
    return (
        <div className={`${css.container} p-5`}>
            <div className="d-flex justify-content-end">
                <button onClick={() => setActiveTab(activeTab === "update" ? "password" : "update")} className="btn btn-link" style={{color: "#4A90E2"}}>
            ...
            {activeTab === "update" && (
                <div>
                <h5 className="mb-4 border-bottom" style={{color: "#4A90E2"}}>Update Details</h5>
                <div>
                    ...
                    <div className="mt-1 d-flex justify-content-center">
                        <button className="btn border w-50" onClick={handleUpdateSubmit}>update</button>
                    </div>
                </div>
                </div>
            )}
            {activeTab === "password" && (
                <div>
                <h5 className="mb-4 border-bottom" style={{color: "#4A90E2"}}>Change Password</h5>
                <div className={`${css.signup_form_field} d-flex gap-3`}>
                    <div className="w-100">
                ...
                <div className="mt-1 d-flex justify-content-center w-100">
                    <button className="btn border w-50" onClick={handlePasswordSubmit}>change</button>
                </div>
            </div>
            )}
        </div>
